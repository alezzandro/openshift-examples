...
oc exec "$EPHEMERAL_POD_NAME" -- bash -c "echo 'Ephemeral content lost on restart!' > /opt/app-root/src/index.html"

# Verify content
oc exec "$EPHEMERAL_POD_NAME" -- cat /opt/app-root/src/index.html
...
oc exec "$NEW_EPHEMERAL_POD_NAME" -- cat /opt/app-root/src/index.html
# Expected output: cat: /opt/app-root/src/index.html: No such file or directory
