...
NODE_NAME_FOR_TAINT="<your-worker-node-name>" 

echo "Tainting node ${NODE_NAME_FOR_TAINT} with app-specific=true:NoSchedule..."
oc taint node "$NODE_NAME_FOR_TAINT" app-specific=true:NoSchedule
...
oc untaint node "$NODE_NAME_FOR_TAINT" app-specific=true:NoSchedule
